//  x_ball, y_ball   - координаты мяча,
//  y_p1  -координата вехней точки ракетки левого игрока,
//  y_p2 - -координата вехней точки ракетки правого игрока
...
int main() {
    ...
    int traf_y = 1;
    while (1) {
        printf("Введите A/Z и K/M для перемещения ракеток или Space Bar для пропуска действия\n");
        ...
        char key;
           scanf("%c", &key);
        if (key == ' ' || key == 'a' || key == 'z' || key == 'k' ||
            key == 'm' || key == 'A' || key == 'Z' || key == 'K' || key == 'M') {
            if ((key == 'a' || key == 'A') && y_p1 >= 2) {
                y_p1 = y_p1 - 1;
            }
            if ((key == 'z' || key == 'Z') && y_p1 <=21) {
                y_p1 = y_p1 + 1;
            }
            if ((key == 'k' || key == 'K') && y_p2 >= 2) {
                y_p2 = y_p2 - 1;
            }
            if ((key == 'm' || key == 'M') && y_p2 <=21) {
                y_p2 = y_p2 + 1;
            }
         y_ball += traf_y;
         x_ball += traf_x;
         if (y_ball == 24 || y_ball == 1) {
             traf_y *= -1;
        ...
        printf("\e[1;1H\e[2J");  // функция очистки экрана как clear
        }
return 0;
}
    ...
    for (int x = 0; x <= 80; x++) {
        if (y == 0 || y == 25) {
            ...
            if (y == y_ball && x == x_ball) {
                printf("%c", 111);
            } else if ((y == y_p1 || y == (y_p1 + 1) || y == (y_p1 + 2)) && x == 1) {
                printf("%c", 124);
            } else if ((y == y_p2 || y == (y_p2 + 1) || y == (y_p2 + 2))  &&  x == 79) {
                printf("%c", 124);
            } else if (x == 0 || x == 80) {
                printf("%c", 124);
            } else if (x == 35 && y == 2) {
